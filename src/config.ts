...
/**
 * Validate an IANA timezone name, throwing early (at startup) on a typo.
 * A bad TZ_NAME would otherwise silently mis-time every cron fire, so we
 * fail loud here rather than drift. Exported for tests.
 */
export function validateTimezone(tz: string): string {
  try {
    ...
    throw new Error(
      `Invalid IANA timezone in TZ_NAME: "${tz}". Use a name like "Africa/Cairo" or "Asia/Riyadh".`,
    );
  }
}
...
export const config = Object.freeze({
  ...
  // Timezone for every cron schedule. Defaults to UTC; throws on a typo.
  timezone: validateTimezone(process.env.TZ_NAME?.trim() || 'UTC'),
