 ...
 * Arabic and Quran text contains chars Markdown/HTML would 400 on (see
 ...
 * already deleted it by hand", so log and move on. Needs the bot's
...
/**
 * Send the evening anonymous self-review poll. Anonymous + multi-answer
 * by default: parents tick the things they did with their kids today,
 * everyone sees aggregate percentages, nobody (not even the bot) learns
 * who voted, no DB, no riya. close_date is clamped to Telegram's accepted
 * range so bad config can't 400 the API. Returns the poll message_id, or
 * null on failure.
 */
